 /*
EDC15 Toolbox, made by Bi0H4z4rD V0.3
   ...
   if (optionset ==2)//Flash operations menu
    ...
    if (optionset ==3)//Tuning operations menu
    {
       ...
       if (optionset == 1)//Add file to the DB
        {
          lcd.clear();
          AddDB(0,0x72000,0x74000,"edc15rd.skf");
          return;
        }
...
void ReadEDC15Info()//Searches in flash for the appropiate data string.
{
  ...
  ReadEDC15FlashBlock(0x72000,0x74000);
  myFile.open("temp", O_READ);
  long offset=StringSearch("EDC",0x00,0x2000,3);
  if (offset==0)
    {
      offset=StringSearch("TDI",0x00,0x2000,3);
    }
  if (offset==0)
    {
      lcd.clear();
 ...
 if (!myFile.open("temp", O_CREAT | O_WRITE)){
  ...
  do{
    ...
    ReadString[6]=iso_checksum(ReadString,6);
     for (byte m=0;m<7; m++)//Send the string
      {
       iso_write_byte(ReadString[m]);
       }
...
lcd.setCursor(0,0);
flp("Erasing....");
for (byte m=0; m<4; m++)//Send the string
  {
    byte WriteString[4]={0x02, 0xA5, 0x0F, 0xB6};
    iso_write_byte(WriteString[m]);
  }
  CheckAck();
...
void WriteEDC15FlashBlock(long BlockStart, long BlockEnd)
{
  ...
  while (BlockEnd>=BlockStart)
  {
    ...
    for (byte sendcount=0; sendcount < 5; sendcount++)//send the header
    {
    ...
    for (byte sendcount=0; sendcount < stringcount; sendcount++)//send the data
    {
     ...
     iso_write_byte(Writebyte[5]);//send the checksumm
     CheckAck();
     if (BlockEnd == 0x7BFFF)
     {
...
void CloseEDC15Ecu()
{
    iso_write_byte(0x01);//Wave byebye to the ecu
...
{
  ReadEDC15FlashBlock(0x72000,0x74000);
  String poopety=AddDB(1,0x00,0x2000,"temp");
  return poopety;
}
...
//Sends the invert of the received bytes
void SendInvert()
{
  iso_read_byte();
  iso_write_byte(~b);
